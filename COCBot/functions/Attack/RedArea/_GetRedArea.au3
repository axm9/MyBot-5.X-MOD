...
Func _GetRedArea()
	...
	If $iMatchMode = $LB And $iChkDeploySettings[$LB] = 5 Then ; Used for DES Side Attack (need to know the side the DES is on)
	...
	ElseIf $iMatchMode = $LB And $iChkDeploySettings[$LB] = 6 Then ; Used for TH Side Attack (need to know the side the TH is on)
	...
	If ($iMatchMode = $LB And $iChkDeploySettings[$LB] = 7) OR ($iMatchMode = $DB and $ichkUseAttackDBCSV = 1) OR ($iMatchMode = $LB and $ichkUseAttackABCSV = 1) Then
